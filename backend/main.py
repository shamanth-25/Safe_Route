...
HYDERABAD_NEIGHBORHOODS = [
    {
        "name": "Madhapur",
        "display_name": "Madhapur, Hyderabad, Telangana, India",
        "lat": 17.4483,
        "lon": 78.3915,
    },
    {
        "name": "Gachibowli",
        "display_name": "Gachibowli, Hyderabad, Telangana, India",
        "lat": 17.4401,
        "lon": 78.3489,
    },
    {
        "name": "Charminar",
        "display_name": "Charminar, Old City, Hyderabad, Telangana, India",
        "lat": 17.3616,
        "lon": 78.4747,
    },
    {
        "name": "Begumpet",
        "display_name": "Begumpet, Hyderabad, Telangana, India",
        "lat": 17.4447,
        "lon": 78.4664,
    },
    {
        "name": "Secunderabad",
        "display_name": "Secunderabad Station, Hyderabad, Telangana, India",
        "lat": 17.4334,
        "lon": 78.5017,
    },
    {
        "name": "Jubilee Hills",
        "display_name": "Jubilee Hills, Hyderabad, Telangana, India",
        "lat": 17.4299,
        "lon": 78.4069,
    },
    {
        "name": "Nampally",
        "display_name": "Nampally Station, Hyderabad, Telangana, India",
        "lat": 17.3924,
        "lon": 78.4682,
    },
    {
        "name": "Hitech City",
        "display_name": "Hitech City, Hyderabad, Telangana, India",
        "lat": 17.4504,
        "lon": 78.3808,
    },
    {
        "name": "Kondapur",
        "display_name": "Kondapur, Hyderabad, Telangana, India",
        "lat": 17.4622,
        "lon": 78.3568,
    },
    {
        "name": "Kukatpally",
        "display_name": "Kukatpally, Hyderabad, Telangana, India",
        "lat": 17.4855,
        "lon": 78.3973,
    },
    {
        "name": "Mehdipatnam",
        "display_name": "Mehdipatnam, Hyderabad, Telangana, India",
        "lat": 17.3912,
        "lon": 78.4357,
    },
    {
        "name": "Banjara Hills",
        "display_name": "Banjara Hills, Hyderabad, Telangana, India",
        "lat": 17.4172,
        "lon": 78.4332,
    },
    {
        "name": "Ameerpet",
        "display_name": "Ameerpet, Hyderabad, Telangana, India",
        "lat": 17.4374,
        "lon": 78.4482,
    },
    {
    ...
    {
        "name": "Tarnaka",
        "display_name": "Tarnaka, Hyderabad, Telangana, India",
        "lat": 17.4292,
        "lon": 78.5382,
    },
    {
    ...
    {
        "name": "Dilsukhnagar",
        "display_name": "Dilsukhnagar, Hyderabad, Telangana, India",
        "lat": 17.3688,
        "lon": 78.5247,
    },
    {
        "name": "Koti",
        "display_name": "Koti, Hyderabad, Telangana, India",
        "lat": 17.3824,
        "lon": 78.4842,
    },
    {
        "name": "Tolichowki",
        "display_name": "Tolichowki, Hyderabad, Telangana, India",
        "lat": 17.3971,
        "lon": 78.4042,
    },
    {
        "name": "Golconda Fort",
        "display_name": "Golconda Fort, Hyderabad, Telangana, India",
        "lat": 17.3833,
        "lon": 78.4011,
    },
    {
        "name": "Durgam Cheruvu",
        "display_name": "Durgam Cheruvu (Secret Lake), Madhapur, Hyderabad, Telangana, India",
        "lat": 17.4325,
        "lon": 78.3888,
    },
    {
        "name": "Durgam Cheruvu Cable Bridge",
        "display_name": "Durgam Cheruvu Cable Bridge, Jubilee Hills, Hyderabad, Telangana, India",
        "lat": 17.4352,
        "lon": 78.3970,
    },
    {
        "name": "Inorbit Mall",
        "display_name": "Inorbit Mall, Madhapur, Hyderabad, Telangana, India",
        "lat": 17.4344,
        "lon": 78.3866,
    },
    {
        "name": "IKEA",
        "display_name": "IKEA Hyderabad, Mindspace, Gachibowli, India",
        "lat": 17.4384,
        "lon": 78.3744,
    },
]
...
def apply_prefix_to_results(results, prefix):
    ...
    for item in results:
        ...
        modified_results.append({**item, "display_name": disp_name})
...
def geocode_recursive(clean_query: str, prefix: str):
    ...
    if not results:
        try:
            ...
            headers = {"User-Agent": "SafePathHyderabad/1.0 (shamanth@saferoute.com)"}
            params = {
    ...
    if not results:
        ...
        if clean_q_alnum:
            for item in HYDERABAD_NEIGHBORHOODS:
                clean_name = "".join(c for c in item["name"].lower() if c.isalnum())
                clean_disp = "".join(
                    c for c in item["display_name"].lower() if c.isalnum()
                )
        ...
        if base_results:
            ...
            for item in base_results:
                ...
                offset_results.append(
                    {**item, "lat": f"{new_lat:.6f}", "lon": f"{new_lon:.6f}"}
                )
...
def get_safest_routes(req: RouteRequest):
    ...
    if ORS_API_KEY and not ORS_API_KEY.startswith("YOUR_") and len(ORS_API_KEY) > 20:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"ORS API returned status code {response.status_code}: {response.text}"
                )
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "routes" in osrm_res and len(osrm_res["routes"]) > 0:
                    ...
                    raise HTTPException(
                        status_code=404, detail="No routes found via OSRM"
                    )
            else:
                raise HTTPException(
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Routing failure (Both ORS and OSRM failed): {str(e)}",
            )
    ...
    for idx, feature in enumerate(routes_data["features"]):
        ...
        def get_facilities_in_buffer(gdf, buf_geom, type_name):
            ...
            for _, row in intersecting_wgs.iterrows():
                ...
                if row.geometry.geom_type == "Point":
                    ...
                    facilities.append(
                        {"name": name, "coordinates": coords, "type": type_name}
                    )
            return facilities
        ...
        route_facilities = {
            ...
            "hospitals": get_facilities_in_buffer(
                hospitals_gdf, buffer_geom, "hospital"
            ),
            "pharmacies": get_facilities_in_buffer(
                pharmacies_gdf, buffer_geom, "pharmacy"
            ),
            "metro": get_facilities_in_buffer(metro_gdf, buffer_geom, "metro"),
            "streetlights": get_facilities_in_buffer(
                streetlights_gdf, buffer_geom, "streetlight"
            )[:100],  # cap streetlight count to prevent map lag
        }
        ...
        def find_nearest(gdf, line_geom, type_name):
            ...
            pt_wgs = (
                gpd.GeoSeries([row.geometry], crs="EPSG:32644")
                .to_crs(epsg=4326)
                .iloc[0]
            )
        ...
        if lights_per_km >= 10:
            reasons.append(
                f"Highly illuminated route with {round(lights_per_km, 1)} streetlights per km."
            )
        elif lights_per_km >= 5:
            reasons.append(
                f"Moderate illumination with {round(lights_per_km, 1)} streetlights per km."
            )
        else:
            reasons.append(
                f"Caution: Low illumination ({round(lights_per_km, 1)} streetlights per km)."
            )

        if counts["police"] > 0:
            reasons.append(
                f"Passed within 200m of {counts['police']} police station{'s' if counts['police'] > 1 else ''} for continuous security."
            )
        else:
        ...
        if counts["hospitals"] > 0 or counts["pharmacies"] > 0:
            reasons.append(
                "Excellent emergency medical support (hospitals & 24/7 pharmacies nearby)."
            )

        if counts["metro"] > 0:
            reasons.append(
                "Provides convenient access to active transit (metro stations)."
            )
    ...
    routes_result = sorted(
        routes_result, key=lambda x: (-x["safety_score"], x["distance_km"])
    )
    ...
    result_payload = {"routes": routes_result, "routing_source": routing_source}
...
async def chat_assistant(req: ChatRequest):
    ...
    if api_key and api_key != "YOUR_GEMINI_API_KEY" and api_key != "YOUR_ORS_API_KEY":
        try:
            ...
            return {"reply": reply_text, "suggested_actions": suggested}
        except Exception as e:
    ...
    if any(
        k in msg for k in ["police", "security", "station", "cop", "safety", "safe"]
    ):
        reply = (
            f"🛡️ **Security Shield Insight:** Your selected path (**{r_name}**) prioritizes safety by routing "
        ...
        suggested = [
            ...
            "What is the emergency helpline?",
            "Streetlight density check",
        ]

    elif any(
        k in msg for k in ["light", "streetlight", "dark", "illuminate", "illumination"]
    ):
        reply = (
            f"💡 **Streetlight Density Analysis:** Our database indexes **14,284 streetlights** in Hyderabad. "
        ...
        suggested = [
            "Check streetlight density",
            "How is safety score calculated?",
            "List nearby police stations",
        ]

    elif any(
        k in msg
        for k in ["hospital", "medical", "pharmacy", "clinic", "doctor", "health"]
    ):
        reply = (
            f"🏥 **Medical Proximity Guide:** Emergency medical support is highly accessible on your path. "
        ...
        suggested = [
            "Locate nearest hospital",
            "Find 24/7 pharmacies",
            "Quick Emergency: 112 info",
        ]

    elif any(k in msg for k in ["transit", "metro", "station", "bus", "train", "cab"]):
        reply = (
            "🚇 **Active Transit Network:** High-density transit points (like Madhapur and Gachibowli Metro Stations) "
            "are heavily illuminated and highly secure, providing rapid emergency evacuation paths and active crowds "
            "during late hours."
        )
        suggested = [
            "Are metro routes safer?",
            "Explain safety ranks",
            "Check streetlight density",
        ]

    elif any(
        k in msg for k in ["help", "emergency", "call", "contact", "112", "phone"]
    ):
        reply = (
            f"🚨 **Emergency Protocol:** If you feel unsafe or experience an emergency, immediately dial **112** "
        ...
        reply = (
            f"👋 **Hello! I am your SafePath AI Safety Assistant.**\n\n"
